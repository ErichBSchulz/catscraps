...
def _get_data(benchmark_data_list: List[BenchmarkData]) -> Tuple[List[str], List[List[float]], List[List[float]], List[List[float]]]:
    ...
    for bd in benchmark_data_list:
        ...
        for name in model_names:
            try:
                ...
                p1.append(0.0); p2.append(0.0); costs.append(0.0)
        all_p1.append(p1); all_p2.append(p2); all_costs.append(costs)
...
def create_plot(
    ...
    if plot_type == "B":
        _create_plot_b(benchmark_data_list, model_names, all_p1, all_p2, all_costs, output_file)
    else:
        _create_plot_a(benchmark_data_list, model_names, all_p1, all_p2, all_costs, show_cost, output_file)

def _create_plot_a(benchmark_data_list, model_names, all_p1, all_p2, all_costs, show_cost, output_file):
    ...
    for i, bd in enumerate(benchmark_data_list):
        ...
        bars = ax.barh(y_pos + offset, widths, bar_height, left=all_p1[i], 
                       label=bd.run_name, color=COLORS[i % len(COLORS)], edgecolor="white", linewidth=0.5)
        if show_cost:
            for j, bar in enumerate(bars):
                if all_costs[i][j] > 0:
                    ax.text(bar.get_x() + bar.get_width() + 0.5, bar.get_y() + bar.get_height()/2, 
                            f"${all_costs[i][j]:.4f}", va='center', fontsize=8, color='#555555')
...
def _create_plot_b(benchmark_data_list, model_names, all_p1, all_p2, all_costs, output_file):
    ...
    for i, bd in enumerate(benchmark_data_list):
        ...
        for j, name in enumerate(model_names):
            ...
            ax.text((p1+p2)/2, cost, f" {name}", va='bottom', ha='center', fontsize=7, alpha=0.7)
    ...
    handles = [Line2D([0], [0], color=COLORS[i % len(COLORS)], lw=4, label=bd.run_name) for i, bd in enumerate(benchmark_data_list)]
